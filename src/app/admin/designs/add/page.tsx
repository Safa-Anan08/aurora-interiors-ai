...
export default function AddDesignPage() {
  ...
  const [img, setImg] = useState('https://images.unsplash.com/photo-1524758631624-e2822e304c36?auto=format&fit=crop&w=600&q=80');
  ...
  // Complex States
  const [colors, setColors] = useState<ColorSwatch[]>([{ name: 'Warm Alabaster', hex: '#f5f5f4' }]);
  const [recommendedProductIds, setRecommendedProductIds] = useState<string[]>([]);
  const [availableProducts, setAvailableProducts] = useState<any[]>([]);
  const [loadingProducts, setLoadingProducts] = useState(false);

  // Fetch products catalog to link
  useEffect(() => {
  ...
  const handleToggleProduct = (prodId: string) => {
    if (recommendedProductIds.includes(prodId)) {
      setRecommendedProductIds(recommendedProductIds.filter(id => id !== prodId));
    } else {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`${SERVER_URL}/api/admin/designs`, payload);
      toast.success('Design concept created and saved into MongoDB!');
      router.push('/admin/designs');
    } catch (err: any) {
      toast.error(err.response?.data?.error?.message || 'Failed to publish design concept.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-sm font-bold text-charcoal uppercase tracking-wider">Publish Design Board</h2>
          <p className="text-[10px] text-gray-400">Add a new dynamic design concept board to the MongoDB database.</p>
        </div>
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        >
          ...
          {submitting ? 'Saving Concept Board...' : 'Save Design Concept'}
        </button>
