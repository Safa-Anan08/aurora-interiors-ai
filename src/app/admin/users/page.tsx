  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel rounded-3xl bg-white border border-lavender-grey/30 overflow-hidden shadow-sm">
                ...
                <tbody>
                  {users.map((user: any) => (
                    ...
                    >
                      ...
                      <td className="p-4">
                        {user.profilePic ? (
                          ...
                          <img
                            src={user.profilePic}
                            alt={user.name}
                            className="w-8 h-8 rounded-full object-cover border border-lavender-grey/20"
                          />
                        ) : (
                      ...
                      <td className="p-4">
                        <span
                          className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider inline-flex items-center gap-1 ${user.status === "active"
                ...
                >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    {user.profilePic ? (
                      ...
                      <img
                        src={user.profilePic}
                        alt={user.name}
                        className="w-12 h-12 rounded-full object-cover border border-lavender-grey/20 shrink-0"
                      />
                    ) : (
                      <div className="w-12 h-12 rounded-full bg-dusty-rose/10 flex items-center justify-center text-dusty-rose font-bold text-lg shrink-0">
                        {user.name?.charAt(0)?.toUpperCase()}
                      </div>
                    )}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {/* Status */}
                        <span
                          className={`shrink-0 px-2 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider inline-flex items-center gap-1 ${user.status === "active"
                              ? "bg-emerald-100 text-emerald-700"
                              : "bg-rose-100 text-rose-700"
                            }`}
                        >
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-lavender-grey/15">
                    ...
                    <div>
                      ...
                      <span
                        className={`inline-flex mt-1 px-2 py-1 rounded text-[10px] font-bold capitalize ${user.role === "admin"
                            ? "bg-dusty-rose/15 text-dusty-rose border border-dusty-rose/25"
                            : "bg-lavender-grey/15 text-lavender-grey-dark border border-lavender-grey/25"
                          }`}
                      >
                  ...
                  <div className="flex items-center justify-end gap-2 mt-4 pt-4 border-t border-lavender-grey/15">
                    ...
                    <button
                      onClick={() => setViewingUser(user)}
                      className="p-2 border border-lavender-grey/35 rounded-lg text-gray-400 hover:text-dusty-rose hover:bg-dusty-rose/5 transition-all"
                      title="View Profile"
                    >
                      <Eye className="w-4 h-4" />
                    </button>
                    ...
                    <button
                      onClick={() => setConfirmDeleteId(user.id)}
                      className="p-2 border border-lavender-grey/35 rounded-lg text-gray-400 hover:text-rose-500 hover:bg-rose-50 transition-all"
                      title="Delete User"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>
        ...
        {/* ================= PAGINATION ================= */}
        {pagination.pages > 1 && (
          <div className="px-4 md:px-6 py-4 flex items-center justify-between border-t border-lavender-grey/25 bg-slate-50/50">
