    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel rounded-3xl bg-white border border-lavender-grey/30 overflow-hidden shadow-sm">
        {isLoading ? (
          ...
          <div className="text-center py-12 text-xs text-gray-500 font-bold">No orders found.</div>
        ) : (
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody>
                {orders.map((order: any) => (
                  <tr key={order._id || order.id} className="border-b border-lavender-grey/20 hover:bg-slate-50/50 transition-colors">
                    <td className="p-4 font-mono font-bold text-charcoal">
                      #{order._id ? order._id.substring(18) : order.id}
                    </td>
                    <td className="p-4 min-w-[120px]">
                      <div className="font-bold text-charcoal">{order.user?.name || 'Guest User'}</div>
                      <div className="text-[10px] text-gray-400 font-medium">{order.user?.email || 'guest@mail.com'}</div>
                    </td>
